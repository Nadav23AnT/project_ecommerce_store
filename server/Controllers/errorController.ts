...
const handleJWTError = () => new AppError('פרטי ההזדהות שגויים, יש להתחבר מחדש!', 401);

const handleJWTExpiredError = () => new AppError('פרטי ההזדהות אינם בתוקף, יש להתחבר מחדש', 401);
...
const globalErrorHandler: ErrorRequestHandler = (err, _req, res, _next) => {
  ...
  if (isDevError) {
    if (error.name === 'PayloadTooLargeError') error.message = 'כמות המידע שהוזנה גדולה מדי';
