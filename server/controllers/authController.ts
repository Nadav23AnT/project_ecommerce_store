...
export const login = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user || !user.correctPassword(password, user.password)) {
      return next(
        new AppError(
          `הסיסמה או כתובת האימייל שהוזנו לא נכונים! נותרו עוד ${req.rateLimit.remaining} נסיונות להתחברות`,
          401
        )
      );
    }
...
// ששלחנו לצד לקוח ולשים בו וכאילו לאפס אותו בצד לקוח cookie כאשר המשתמש מתנתק אני רוצה לדרוס את ה
// 'loggedout' מחרוזת ריקה או 'token' key למשל על ידי שאני ישים ב
